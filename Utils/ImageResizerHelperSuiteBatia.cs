...
public static class ImageResizerHelperSuiteBatia {

    static ImageResizerHelperSuiteBatia() {
    }
...
public static byte[] ForceVertical(byte[] imageData)
{
    try
    {
        ...
        try
        {
            // Si está horizontal → rotar a vertical
            if (bmp.Width > bmp.Height)
            {
                Matrix matrix = new Matrix();
                matrix.PostRotate(90); // ← giro a la derecha

                rotated = Bitmap.CreateBitmap(
                    bmp, 0, 0, bmp.Width, bmp.Height, matrix, true
                );

                // Liberar BMP original
                bmp.Recycle();
                bmp.Dispose();
                bmp = null;
            }
            ...
            {
                // Ya es vertical → regresar la original
                return imageData;
            }

            using (var ms = new MemoryStream())
            {
                rotated.Compress(Bitmap.CompressFormat.Jpeg, 100, ms);
                return ms.ToArray();
            }
        }
        finally
        {
            try { rotated?.Recycle(); rotated?.Dispose(); } catch { }
            try { bmp?.Recycle(); bmp?.Dispose(); } catch { }
        }
    }
    catch
    {
        // ❗️Si ocurre cualquier error, regresa la imagen original
        return imageData;
    }
}
