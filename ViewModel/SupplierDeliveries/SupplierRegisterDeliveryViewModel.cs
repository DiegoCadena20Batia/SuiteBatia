...
public partial class SupplierRegisterDeliveryViewModel : BaseViewModel, IQueryAttributable {
    ...
    public SupplierRegisterDeliveryViewModel(DrawingView drawingView) {
    ...
    async Task GuardarLocal() {
        try {
            ...
            //DetenerCarga();
            await DisplayAlert("Aviso", "Sin conexión a internet, la entrega se guardó en el dispositivo para un envío posterior", "Ok");
            IsBusy = false;
            await Shell.Current.GoToAsync("//MyMenu");
            return;
        } catch(Exception ex) {
