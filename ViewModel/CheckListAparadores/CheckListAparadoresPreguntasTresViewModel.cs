...
public partial class CheckListAparadoresPreguntasTresViewModel : ViewModelBase {
    ...
    public CheckListAparadoresPreguntasTresViewModel(CheckListService checkListService) {
        _checkListService = checkListService;

        var items = _checkListService.GetPreguntasBySeccion(3);
    ...
    public void CargarFotosGuardadasPorSeccion() {
        PhotoList.Clear();
        PhotoList = new ObservableCollection<string>(
            _checkListService.ObtenerFotosPorSeccion(3).Select(x => x.Path));
    }
