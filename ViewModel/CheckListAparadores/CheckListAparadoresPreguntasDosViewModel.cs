...
public partial class CheckListAparadoresPreguntasDosViewModel : ViewModelBase {
    [ObservableProperty]
    bool _isLoading;
    ...
    public CheckListAparadoresPreguntasDosViewModel(CheckListService checkListService) {
        _checkListService = checkListService;

        var items = _checkListService.GetPreguntasBySeccion(2);
    ...
    async Task Continuar() {
        ...
        IsLoading = true;
        await Constants.GoToAsync(nameof(CheckListAparadoresPreguntasTresPage));
        IsLoading = false;
    }
    ...
    public void CargarFotosGuardadasPorSeccion() {
        PhotoList.Clear();
        PhotoList = new ObservableCollection<string>(
            _checkListService.ObtenerFotosPorSeccion(2).Select(x => x.Path));
    }
