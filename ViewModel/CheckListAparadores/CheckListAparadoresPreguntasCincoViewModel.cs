...
public partial class CheckListAparadoresPreguntasCincoViewModel : ViewModelBase {
    ...
    public CheckListAparadoresPreguntasCincoViewModel(CheckListService checkListService) {
        _checkListService = checkListService;

        var items = _checkListService.GetPreguntasBySeccion(5);
    ...
    async Task Continuar() {
        ...
        //IR A PAGINA FINAL DE REVIEW DE CHECKLIST Y FIRMAS
        await Constants.GoToAsync(nameof(CheckListAparadoresPreguntasResumenPage));
        //await Shell.Current.GoToAsync(nameof(ResumenCheckListPage));
        IsLoading = false;
    }
    ...
    public void CargarFotosGuardadasPorSeccion() {
        PhotoList.Clear();
        PhotoList = new ObservableCollection<string>(
            _checkListService.ObtenerFotosPorSeccion(5).Select(x => x.Path));
    }
